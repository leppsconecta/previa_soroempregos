    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const mappedJob: Job = {
                ...
                postedAt: new Date(data.created_at).toLocaleDateString('pt-BR') + ' às ' + new Date(data.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
                ...
                status_anunciante: data.status_anunciante,
                cta_public_contato: data.cta_public_contato,
                cta_public_email: data.cta_public_email,
                cta_public_link: data.cta_public_link,
                cta_public_endereco: data.cta_public_endereco,
                cta_public_observations_whatsapp: data.cta_public_observations_whatsapp,
                cta_public_observations_email: data.cta_public_observations_email,
                cta_public_observations_link: data.cta_public_observations_link,
                cta_public_observations_endereco: data.cta_public_observations_endereco,
                companyData: {
                    name: data.company_name,
                    profile_header_color: data.profile_header_color,
        ...
        <div className="bg-slate-50 min-h-screen font-sans">
            ...
            <section className="bg-yellow-400 py-10 md:py-16 px-6 md:px-12 lg:px-24 rounded-b-[3rem] shadow-xl relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold text-blue-950 leading-[1.1] mb-6">
                        Encontre seu próximo<br className="hidden md:block" /> emprego <span className="text-white drop-shadow-sm font-black">agora</span>.
                    </h1>
                    <p className="text-base md:text-xl text-blue-900/70 font-medium mb-10 max-w-lg mx-auto leading-relaxed">
                    ...
                    <div className="max-w-sm md:max-w-md mx-auto flex flex-col md:block gap-3">
                        ...
                        <div className="bg-white p-1.5 rounded-2xl shadow-xl shadow-blue-900/5 flex items-center transform transition-all focus-within:scale-[1.02] focus-within:shadow-2xl focus-within:shadow-blue-900/10 border border-white/20">
                            <input
                                ...
                                placeholder="#1234"
                                className="flex-1 h-12 md:h-14 px-4 text-center text-xl font-semibold text-blue-950 placeholder:text-slate-200 outline-none bg-transparent uppercase tracking-widest placeholder:font-normal"
                            />
                            {/* Button inline — only on md+ */}
                            <button
                                onClick={handleSearch}
                                disabled={loading}
                                className="hidden md:flex bg-blue-950 text-white h-14 px-8 rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-blue-900 transition-all disabled:opacity-70 items-center justify-center shrink-0 shadow-lg shadow-blue-950/20 active:scale-95 gap-2"
                            >
            ...
            <section className="py-6 md:py-12 px-4 md:px-8 max-w-7xl mx-auto">
                ...
                <div className="flex justify-center md:hidden mb-4 md:mb-12">
                    <Link
                        to="/vagas"
                        className="w-full max-w-sm py-4 rounded-xl bg-blue-100 text-blue-700 font-semibold flex items-center justify-center gap-2 hover:bg-blue-200 transition-colors"
                    >
                        Ver todas as vagas <ArrowRight size={20} />
                    </Link>
                </div>
            ...
            {foundJob && (
                <>
                    ...
                    <ApplicationModal
                        ...
                        isAdvertiser={!!(foundJob as any).status_anunciante}
                        advertiserName={(foundJob as any).anunciante}
                        ctaContato={(foundJob as any).cta_public_contato}
                        ctaEmail={(foundJob as any).cta_public_email}
                        ctaLink={(foundJob as any).cta_public_link}
                        ctaEndereco={(foundJob as any).cta_public_endereco}
                        ctaObservationsWhatsapp={(foundJob as any).cta_public_observations_whatsapp}
